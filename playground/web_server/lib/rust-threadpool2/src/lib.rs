    ...
    fn drop(&mut self) {
        if self.active {
            *self.thread_counter.lock().unwrap() -= 1;
            spawn_in_pool(self.jobs.clone(), self.thread_counter.clone(), self.thread_count_max.clone(), thread::current().name().unwrap().parse::<u8>().unwrap(),self.tx_log.clone())
        }
    }
}
...
impl ThreadPool {
    ...
    pub fn new(threads: usize, tx_log: Sender<i32>) -> ThreadPool {
        assert!(threads >= 1);
        ...
        // Threadpool threads
        for id in 0..threads {
            spawn_in_pool(rx.clone(), active_count.clone(), max_count.clone(), id.clone() as u8, tx_log.clone());
        }
        ...
        *self.max_count.lock().unwrap() = threads;
        if threads > current_max {
            ...
            for id in 0..(threads - current_max) {
                spawn_in_pool(self.job_receiver.clone(), self.active_count.clone(), self.max_count.clone(), (current_max + id).clone() as u8, self.tx_log.clone());
            }
        }
    }
}
...
fn spawn_in_pool(jobs: Arc<Mutex<Receiver<Thunk<'static>>>>,
        ...
        let mut sentinel = Sentinel::new(&jobs, &thread_counter, &thread_count_max, tx_log);
